...
<template>
    <div class="bg-white text-gray-900">
        <header class="flex items-start gap-5 border-b border-gray-200 p-8">
            <img
            ...
            <div>
                ...
                <p v-if="resume.personal.title" class="mt-0.5 text-sm font-medium text-emerald-600">
        ...
        <div class="grid grid-cols-3">
            <!-- Main -->
            <main class="col-span-2 space-y-5 bg-white p-6">
            ...
            <!-- Right rail -->
            <aside class="col-span-1 space-y-5 bg-gray-50 p-6">
                <section v-if="resume.skills.length">
                    <h2 class="text-xs font-bold uppercase tracking-wide text-gray-400">Skills</h2>
                    <div class="mt-2 flex flex-wrap gap-1.5">
                        <span v-for="skill in resume.skills" :key="skill" class="rounded-full bg-white px-2.5 py-0.5 text-xs font-medium text-gray-700 ring-1 ring-gray-200">
                            {{ skill }}
                        </span>
                    </div>
                </section>
                ...
                <section v-if="hasLanguages">
                    <h2 class="text-xs font-bold uppercase tracking-wide text-gray-400">Languages</h2>
                    <ul class="mt-2 space-y-0.5 text-xs text-gray-700">
                ...
                <section v-if="hasCertifications">
                    <h2 class="text-xs font-bold uppercase tracking-wide text-gray-400">Certifications</h2>
                ...
                <section v-if="hasTrainings">
                    ...
                    <ul class="mt-2 space-y-1 text-xs text-gray-700">
                        <li v-for="entry in resume.trainings.filter((e) => e.name)" :key="entry.id">
                ...
                <section v-if="hasAwards">
                    ...
                    <ul class="mt-2 space-y-1 text-xs text-gray-700">
                        <li v-for="entry in resume.awards.filter((e) => e.name)" :key="entry.id">
                            {{ entry.name }}<span v-if="entry.organization"> — {{ entry.organization }}</span>
                        </li>
                    </ul>
                </section>
                ...
                <section v-if="hasVolunteer">
                    ...
                    <ul class="mt-2 space-y-1 text-xs text-gray-700">
                        <li v-for="entry in resume.volunteer.filter((e) => e.organization || e.role)" :key="entry.id">
