...
<template>
    <div class="bg-white text-gray-900">
        <header class="relative flex items-center gap-5 bg-teal-600 p-8">
            <img
                v-if="resume.personal.photo"
                :src="resume.personal.photo"
                alt="Profile photo"
                class="size-16 shrink-0 rounded-full object-cover ring-2 ring-white/50"
            />
            <div>
                <h1 class="text-2xl font-bold tracking-tight text-white">{{ resume.personal.fullName || 'Your Name' }}</h1>
                <p v-if="resume.personal.title" class="mt-0.5 text-sm font-medium text-teal-50">
                    {{ resume.personal.title }}
                </p>
                <p v-if="contactLine.length" class="mt-2 text-xs leading-relaxed text-teal-50/80">
                    {{ contactLine.join(' · ') }}
                </p>
            </div>
            <span class="absolute -bottom-3 right-8 rotate-6 rounded bg-white px-2 py-0.5 text-[10px] font-bold uppercase tracking-widest text-teal-700 shadow">
                Confirmed
            </span>
        </header>

        <div class="relative border-t-2 border-dashed border-gray-300">
            <span class="absolute -left-3 -top-3 size-6 rounded-full bg-gray-50"></span>
            <span class="absolute -right-3 -top-3 size-6 rounded-full bg-gray-50"></span>
        </div>

        <div class="p-10">
            <section v-if="resume.summary">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">Summary</h2>
                <p class="mt-1.5 text-sm leading-relaxed text-gray-700">{{ resume.summary }}</p>
            </section>

            <section v-if="hasExperience" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">Experience</h2>
            ...
            <section v-if="hasEducation" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">Education</h2>
            ...
            <section v-if="resume.skills.length" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">Skills</h2>
                <div class="mt-2 flex flex-wrap gap-1.5">
                    <span v-for="skill in resume.skills" :key="skill" class="rounded-full bg-teal-50 px-2.5 py-0.5 text-xs font-medium text-teal-700">
                        {{ skill }}
                    </span>
                </div>
            </section>
            ...
            <section v-if="hasProjects" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">Projects</h2>
                <div class="mt-2 space-y-2">
            ...
            <section v-if="hasCertifications" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">Certifications</h2>
            ...
            <section v-if="hasTrainings" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">Trainings & Seminars</h2>
                <div class="mt-2 space-y-2">
            ...
            <section v-if="hasAwards" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">Awards & Achievements</h2>
                <div class="mt-2 space-y-2">
            ...
            <section v-if="hasVolunteer" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">Volunteer Experience</h2>
                <div class="mt-2 space-y-2">
            ...
            <section v-if="hasLanguages" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">Languages</h2>
            ...
            <section v-for="section in visibleCustomSections" :key="section.id" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">{{ section.title || 'Additional Information' }}</h2>
                <div class="mt-2 space-y-2">
            ...
            <section v-if="hasReferences" class="mt-5">
                <h2 class="text-xs font-bold uppercase tracking-wide text-teal-600">References</h2>
                <div class="mt-2 space-y-1">
