...
<template>
    <div class="bg-white text-gray-900">
        <header class="flex items-center gap-6 bg-indigo-600 p-8 text-white">
            <img
                v-if="resume.personal.photo"
                :src="resume.personal.photo"
                alt="Profile photo"
                class="size-24 shrink-0 rounded-full object-cover ring-4 ring-white/30"
            />
            <div>
                <h1 class="text-2xl font-bold tracking-tight">{{ resume.personal.fullName || 'Your Name' }}</h1>
                <p v-if="resume.personal.title" class="mt-1 text-sm font-medium text-indigo-100">
                    {{ resume.personal.title }}
                </p>
                <p v-if="contactLine.length" class="mt-2 text-xs leading-relaxed text-indigo-100">
                    {{ contactLine.join(' · ') }}
                </p>
            </div>
        </header>
        ...
        <div class="p-10">
                ...
                <div class="mt-2 space-y-3">
                    <div v-for="entry in resume.experience" :key="entry.id">
                        ...
                        <p v-if="entry.achievements" class="mt-1 whitespace-pre-line text-sm leading-relaxed text-gray-700">
                            <span class="font-medium text-indigo-600">Achievements: </span>{{ entry.achievements }}
                        </p>
                    </div>
                </div>
            </section>
            ...
            <section v-if="resume.skills.length" class="mt-5">
                ...
                <div class="mt-2 flex flex-wrap gap-1.5">
                    <span v-for="skill in resume.skills" :key="skill" class="rounded-full bg-indigo-50 px-2.5 py-0.5 text-xs font-medium text-indigo-700">
                        {{ skill }}
                    </span>
                </div>
            </section>
