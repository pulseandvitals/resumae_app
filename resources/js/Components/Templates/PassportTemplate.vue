...
<template>
    <div class="bg-white p-10 text-gray-900">
        <header class="flex items-center gap-4 rounded-lg border-2 border-blue-900 bg-blue-50 p-4">
            <img
                v-if="resume.personal.photo"
                :src="resume.personal.photo"
                alt="Profile photo"
                class="size-16 shrink-0 rounded border border-blue-900 object-cover"
            />
            <div>
                <h1 class="text-xl font-bold text-blue-950">{{ resume.personal.fullName || 'Your Name' }}</h1>
                <p v-if="resume.personal.title" class="mt-0.5 text-sm font-medium text-blue-700">
                    {{ resume.personal.title }}
                </p>
                <p v-if="contactLine.length" class="mt-2 text-xs leading-relaxed text-blue-800">
                    {{ contactLine.join(' · ') }}
                </p>
            </div>
            <span class="ml-auto rotate-[-8deg] rounded border-2 border-blue-700 px-2 py-1 text-[10px] font-bold uppercase tracking-widest text-blue-700">
                Verified
            </span>
        </header>

        <section v-if="resume.summary" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">Summary</h2>
            <p class="mt-1.5 text-sm leading-relaxed text-gray-700">{{ resume.summary }}</p>
        </section>

        <section v-if="hasExperience" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">Experience</h2>
        ...
        <section v-if="hasEducation" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">Education</h2>
        ...
        <section v-if="resume.skills.length" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">Skills</h2>
        ...
        <section v-if="hasProjects" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">Projects</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasCertifications" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">Certifications</h2>
        ...
        <section v-if="hasTrainings" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">Trainings & Seminars</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasAwards" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">Awards & Achievements</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasVolunteer" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">Volunteer Experience</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasLanguages" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">Languages</h2>
        ...
        <section v-for="section in visibleCustomSections" :key="section.id" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">{{ section.title || 'Additional Information' }}</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasReferences" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-blue-800">References</h2>
            <div class="mt-2 space-y-1">
