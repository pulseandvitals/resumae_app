...
<template>
    <div class="bg-gray-950 p-10 text-gray-100">
        <header class="flex items-start gap-5 border-b border-gray-800 pb-4">
            <img
            ...
            />
            <div>
                <h1 class="text-2xl font-bold text-fuchsia-400" style="text-shadow: 0 0 12px rgba(232,121,249,0.8);">{{ resume.personal.fullName || 'Your Name' }}</h1>
                <p v-if="resume.personal.title" class="mt-0.5 text-sm font-medium text-cyan-300">
        ...
        <section v-if="resume.summary" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">Summary</h2>
            <p class="mt-1.5 text-sm leading-relaxed text-gray-300">{{ resume.summary }}</p>
        </section>

        <section v-if="hasExperience" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">Experience</h2>
            <div class="mt-2 space-y-3">
                <div v-for="entry in resume.experience" :key="entry.id">
                    ...
                    <p v-if="entry.responsibilities" class="mt-1 whitespace-pre-line text-sm leading-relaxed text-gray-300">
                        {{ entry.responsibilities }}
                    </p>
                    <p v-if="entry.achievements" class="mt-1 whitespace-pre-line text-sm leading-relaxed text-gray-300">
        ...
        <section v-if="hasEducation" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">Education</h2>
            <div class="mt-2 space-y-2">
                <div v-for="entry in resume.education" :key="entry.id">
                    <div v-if="entry.school || entry.degree" class="flex items-baseline justify-between gap-2">
                        <p class="text-sm font-semibold text-gray-100">{{ entry.school }}</p>
                    ...
                    <p v-if="entry.description" class="mt-1 text-sm leading-relaxed text-gray-300">{{ entry.description }}</p>
                </div>
            </div>
        </section>

        <section v-if="resume.skills.length" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">Skills</h2>
            <div class="mt-2 flex flex-wrap gap-1.5">
                <span v-for="skill in resume.skills" :key="skill" class="rounded-full bg-gray-800 px-2.5 py-0.5 text-xs font-medium text-fuchsia-300">
                    {{ skill }}
                </span>
            </div>
        </section>
        ...
        <section v-if="hasProjects" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">Projects</h2>
            <div class="mt-2 space-y-2">
                <div v-for="entry in resume.projects" :key="entry.id">
                    ...
                    <p v-if="entry.description" class="text-sm leading-relaxed text-gray-300">{{ entry.description }}</p>
                </div>
            </div>
        </section>

        <section v-if="hasCertifications" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">Certifications</h2>
        ...
        <section v-if="hasTrainings" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">Trainings & Seminars</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasAwards" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">Awards & Achievements</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasVolunteer" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">Volunteer Experience</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasLanguages" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">Languages</h2>
        ...
        <section v-for="section in visibleCustomSections" :key="section.id" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">{{ section.title || 'Additional Information' }}</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasReferences" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-cyan-300" style="text-shadow: 0 0 8px rgba(103,232,249,0.6);">References</h2>
            <div class="mt-2 space-y-1">
                <p v-for="entry in resume.references" :key="entry.id" class="text-sm text-gray-300">
