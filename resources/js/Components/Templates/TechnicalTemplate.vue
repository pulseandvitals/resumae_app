...
<template>
    <div class="bg-white p-10 text-gray-900">
        <header class="flex items-center gap-4 border-b-2 border-gray-900 pb-4">
            <img
            ...
            />
            <div>
                <h1 class="font-mono text-xl font-bold text-gray-900">{{ resume.personal.fullName || 'Your Name' }}</h1>
                <p v-if="resume.personal.title" class="mt-0.5 font-mono text-sm text-teal-600">&gt; {{ resume.personal.title }}</p>
                <p v-if="contactLine.length" class="mt-2 font-mono text-[11px] text-gray-500">{{ contactLine.join('  |  ') }}</p>
            </div>
        </header>

        <section v-if="resume.summary" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># summary</h2>
            <p class="mt-1.5 text-sm leading-relaxed text-gray-700">{{ resume.summary }}</p>
        </section>

        <section v-if="resume.skills.length" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># skills</h2>
            <div class="mt-2 rounded-md bg-gray-900 p-3">
                <p class="break-words font-mono text-xs leading-relaxed text-teal-400">
                    <span v-for="(skill, index) in resume.skills" :key="skill">{{ skill }}<span v-if="index < resume.skills.length - 1" class="text-gray-500">, </span></span>
                </p>
            </div>
        </section>

        <section v-if="hasExperience" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># experience</h2>
            <div class="mt-2 space-y-3">
                <div v-for="entry in resume.experience" :key="entry.id">
                    <div v-if="entry.position || entry.company" class="flex items-baseline justify-between gap-2">
                        ...
                        <p class="shrink-0 font-mono text-xs text-gray-400">{{ dateRange(entry.startDate, entry.endDate, entry.current) }}</p>
                    </div>
                    ...
                    <p v-if="entry.achievements" class="mt-1 whitespace-pre-line text-sm leading-relaxed text-gray-700">
                        <span class="font-mono text-xs text-teal-600">achievements:</span> {{ entry.achievements }}
                    </p>
                </div>
            </div>
        </section>
        ...
        <section v-if="hasProjects" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># projects</h2>
            <div class="mt-2 space-y-2">
                <div v-for="entry in resume.projects" :key="entry.id">
                    <div v-if="entry.name" class="flex items-baseline justify-between gap-2">
                        ...
                        <p class="shrink-0 font-mono text-xs text-gray-400">{{ dateRange(entry.startDate, entry.endDate) }}</p>
                    </div>
        ...
        <section v-if="hasEducation" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># education</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasCertifications" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># certifications</h2>
            <div class="mt-2 space-y-1">
                <div v-for="entry in resume.certifications" :key="entry.id" class="flex items-baseline justify-between gap-2">
                    ...
                    <p v-if="entry.date" class="shrink-0 font-mono text-xs text-gray-400">{{ formatMonth(entry.date) }}</p>
                </div>
            </div>
        </section>

        <section v-if="hasTrainings" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># trainings</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasAwards" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># awards</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasVolunteer" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># volunteer</h2>
            <div class="mt-2 space-y-2">
        ...
        <section v-if="hasLanguages" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># languages</h2>
        ...
        <section v-for="section in visibleCustomSections" :key="section.id" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># {{ (section.title || 'additional info').toLowerCase() }}</h2>
            <div class="mt-2 space-y-2">
                <div v-for="item in section.items.filter((entry) => entry.title)" :key="item.id">
                    <div class="flex items-baseline justify-between gap-2">
                        ...
                        <p v-if="item.date" class="shrink-0 font-mono text-xs text-gray-400">{{ item.date }}</p>
                    </div>
        ...
        <section v-if="hasReferences" class="mt-5">
            <h2 class="font-mono text-xs font-bold uppercase tracking-wide text-teal-600"># references</h2>
            <div class="mt-2 space-y-1">
