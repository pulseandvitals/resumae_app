...
<template>
    <div class="bg-white p-10 text-gray-900">
        <header class="flex items-center gap-4 rounded-2xl bg-sky-50 p-5">
            <img
            ...
            />
            <div>
                <h1 class="text-xl font-bold text-gray-900">{{ resume.personal.fullName || 'Your Name' }}</h1>
                <p v-if="resume.personal.title" class="mt-0.5 text-sm font-medium text-sky-600">{{ resume.personal.title }}</p>
                <p v-if="contactLine.length" class="mt-2 text-xs text-gray-500">{{ contactLine.join(' · ') }}</p>
            </div>
        </header>

        <section v-if="resume.summary" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-sky-600">About Me</h2>
            <p class="mt-1.5 text-sm leading-relaxed text-gray-700">{{ resume.summary }}</p>
        </section>

        <!-- Education first: this template leads with schooling for students & new grads -->
        <section v-if="hasEducation" class="mt-5">
            <h2 class="text-xs font-bold uppercase tracking-wide text-sky-600">Education</h2>
            <div class="mt-2 space-y-2">
                <div v-for="entry in resume.education" :key="entry.id" class="rounded-lg bg-emerald-50 p-3">
                    <div v-if="entry.school || entry.degree" class="flex items-baseline justify-between gap-2">
                        ...
                        <p class="shrink-0 text-xs text-emerald-600">{{ dateRange(entry.startDate, entry.endDate) }}</p>
                    </div>
            ...
            <div class="mt-2 space-y-3">
                <div v-for="entry in resume.experience" :key="entry.id">
                    ...
                    <p v-if="entry.achievements" class="mt-1 whitespace-pre-line text-sm leading-relaxed text-gray-700">
                        <span class="font-medium text-sky-600">Achievements: </span>{{ entry.achievements }}
                    </p>
                </div>
            </div>
        </section>
        ...
        <section v-if="resume.skills.length" class="mt-5">
            ...
            <div class="mt-2 flex flex-wrap gap-1.5">
                <span v-for="skill in resume.skills" :key="skill" class="rounded-full bg-sky-100 px-2.5 py-0.5 text-xs font-medium text-sky-700">
                    {{ skill }}
                </span>
            </div>
        </section>
